...
async function init(){if(!pool){console.log('DATABASE_URL not set: using temporary memory store');return}await pool.query(`CREATE TABLE IF NOT EXISTS users(id text PRIMARY KEY,username text UNIQUE NOT NULL,display_name text NOT NULL,avatar text DEFAULT '',friends jsonb DEFAULT '[]',created_at bigint NOT NULL);CREATE TABLE IF NOT EXISTS messages(id text PRIMARY KEY,"from" text NOT NULL,"to" text NOT NULL,text text NOT NULL,time bigint NOT NULL);CREATE TABLE IF NOT EXISTS groups_data(id text PRIMARY KEY,name text NOT NULL,owner text NOT NULL,members jsonb DEFAULT '[]',created_at bigint NOT NULL);ALTER TABLE groups_data ADD COLUMN IF NOT EXISTS username text;`);await pool.query("UPDATE groups_data SET username=lower(regexp_replace(name,'[^a-zA-Z0-9_.-]','','g'))||'group' WHERE username IS NULL");await pool.query("CREATE UNIQUE INDEX IF NOT EXISTS groups_data_username_uq ON groups_data(username) WHERE username IS NOT NULL");console.log('Database ready')}
...
async function deleteAccount(username){
 ...
 if(!pool){
  mem.messages=mem.messages.filter(m=>m.from!==username&&m.to!==username);
  mem.groups=mem.groups.filter(g=>g.owner!==username);
  mem.groups.forEach(g=>g.members=(g.members||[]).filter(x=>x!==username));
  mem.users.forEach(u=>u.friends=(u.friends||[]).filter(x=>x!==username));
  mem.users=mem.users.filter(u=>u.username!==username);
  return;
 }
...
async function allGroups(){if(!pool)return mem.groups;const r=await pool.query('SELECT * FROM groups_data ORDER BY created_at ASC');return r.rows.map(x=>({id:x.id,name:x.name,username:x.username||'',owner:x.owner,members:Array.isArray(x.members)?x.members:[],createdAt:Number(x.created_at)}))}
async function addGroup(g){if(!pool){mem.groups.push(g);return}await pool.query('INSERT INTO groups_data(id,name,username,owner,members,created_at) VALUES($1,$2,$3,$4,$5,$6)',[g.id,g.name,g.username,g.owner,JSON.stringify(g.members),g.createdAt])}
...
if(req.url==='/api/delete-account'&&req.method==='POST'){const b=await body(req),username=String(b.username||'').replace(/^@/,'').trim().toLowerCase();if(!username)return json(res,400,{error:'Username required'});const u=await findUser(username);if(!u)return json(res,404,{error:'Пользователь не найден'});await deleteAccount(username);return json(res,200,{ok:true})}
if(req.url==='/api/users'&&req.method==='GET')return json(res,200,{users:await users()});
if(req.url==='/api/snapshot')return json(res,200,{users:await users(),messages:await allMessages(),groups:await allGroups()});if(req.url.startsWith('/api/groups/search')){const q=new URL(req.url,'http://localhost').searchParams.get('q')?.trim().toLowerCase()||'';const gs=await allGroups();const score=n=>{n=n.toLowerCase();if(!q)return 0;if(n===q)return 100;if(n.startsWith(q))return 90;if(n.includes(q))return 75;return Math.round(([...q].filter(c=>n.includes(c)).length/Math.max(q.length,1))*60)};return json(res,200,{groups:gs.map(g=>({...g,score:score(g.name)})).filter(g=>g.score>0).sort((a,b)=>b.score-a.score).slice(0,50)})}
...
if(m.type==='avatar-update'){const avatar=String(m.avatar||'');if(avatar.length>2_000_000)return ws.send(JSON.stringify({type:'error',message:'Аватар слишком большой'}));await updateAvatar(ws.user.username,avatar);ws.user.avatar=avatar;sendTo(ws.user.username,{type:'avatar-updated',avatar});return}
if(m.type==='message'){const to=String(m.to||'').replace(/^@/,'').trim().toLowerCase(),text=String(m.text||'').trim().slice(0,4000);if(!to||!text)return;const me=await findUser(ws.user.username);if(to.startsWith('group:')){const gid=to.slice(6),gs=await allGroups(),g=gs.find(x=>x.id===gid);if(!g||!(g.members||[]).includes(ws.user.username))return ws.send(JSON.stringify({type:'error',message:'Сначала войди в группу'}));const msg={id:crypto.randomUUID(),from:ws.user.username,to,text,time:Date.now()};await addMessage(msg);for(const member of g.members||[])sendTo(member,{type:'message',message:msg});return}if(!(me.friends||[]).includes(to))return ws.send(JSON.stringify({type:'error',message:'Сначала добавь пользователя в друзья'}));const msg={id:crypto.randomUUID(),from:ws.user.username,to,text,time:Date.now()};await addMessage(msg);sendTo(to,{type:'message',message:msg});ws.send(JSON.stringify({type:'message',message:msg}));return}
if(m.type==='group-create'){const name=String(m.name||'').trim().slice(0,50),username=String(m.username||'').replace(/^@/,'').trim().toLowerCase();if(!name||!/^[a-z0-9_.-]{3,24}group$/.test(username))return ws.send(JSON.stringify({type:'error',message:'Юзер группы должен заканчиваться на group'}));const gs=await allGroups();if(gs.some(g=>g.username===username))return ws.send(JSON.stringify({type:'error',message:'Юзер группы уже занят'}));const g={id:crypto.randomUUID(),name,username,owner:ws.user.username,members:[ws.user.username],createdAt:Date.now()};await addGroup(g);ws.send(JSON.stringify({type:'group',group:g}));return}
if(m.type==='group-join'){const id=String(m.id||''),gs=await allGroups(),g=gs.find(x=>x.id===id);if(!g)return ws.send(JSON.stringify({type:'error',message:'Группа не найдена'}));if(!g.members.includes(ws.user.username))g.members.push(ws.user.username);if(pool)await pool.query('UPDATE groups_data SET members=$1 WHERE id=$2',[JSON.stringify(g.members),g.id]);else{const mg=mem.groups.find(x=>x.id===id);if(mg)mg.members=g.members}ws.send(JSON.stringify({type:'group',group:g}));return}
