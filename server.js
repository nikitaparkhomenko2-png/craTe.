...
if(m.type==='group-create'){const name=String(m.name||'').trim().slice(0,50);if(!name)return;const g={id:crypto.randomUUID(),name,owner:ws.user.username,members:[ws.user.username],createdAt:Date.now()};await addGroup(g);for(const [x] of clients)if(x.readyState===WebSocket.OPEN)x.send(JSON.stringify({type:'group',group:g}));return}
if(m.type==='group-join'){const id=String(m.id||'');const gs=await allGroups();const g=gs.find(x=>x.id===id);if(!g)return ws.send(JSON.stringify({type:'error',message:'Группа не найдена'}));if(!g.members.includes(ws.user.username)){g.members.push(ws.user.username);if(pool)await pool.query('UPDATE groups_data SET members=$1 WHERE id=$2',[JSON.stringify(g.members),g.id]);else{const mg=mem.groups.find(x=>x.id===id);if(mg)mg.members=g.members}};ws.send(JSON.stringify({type:'group',group:g}));return}
