...
async function init(){if(!pool){console.log('DATABASE_URL not set: using temporary memory store');return}await pool.query(`CREATE TABLE IF NOT EXISTS users(id text PRIMARY KEY,username text UNIQUE NOT NULL,display_name text NOT NULL,avatar text DEFAULT '',friends jsonb DEFAULT '[]',created_at bigint NOT NULL);CREATE TABLE IF NOT EXISTS messages(id text PRIMARY KEY,"from" text NOT NULL,"to" text NOT NULL,text text NOT NULL,time bigint NOT NULL);CREATE TABLE IF NOT EXISTS groups_data(id text PRIMARY KEY,name text NOT NULL,username text UNIQUE NOT NULL,owner text NOT NULL,members jsonb DEFAULT '[]',created_at bigint NOT NULL);ALTER TABLE groups_data ADD COLUMN IF NOT EXISTS username text;`);console.log('Database ready')}
...
async function allGroups(){if(!pool)return mem.groups;const r=await pool.query('SELECT * FROM groups_data ORDER BY created_at ASC');return r.rows.map(x=>({id:x.id,name:x.name,username:x.username||'',owner:x.owner,members:x.members,createdAt:Number(x.created_at)}))}
async function clearChat(a,b){
 if(!pool){mem.messages=mem.messages.filter(m=>!((m.from===a&&m.to===b)||(m.from===b&&m.to===a)));return}
 await pool.query('DELETE FROM messages WHERE ("from"=$1 AND "to"=$2) OR ("from"=$2 AND "to"=$1)',[a,b]);
}
async function deleteGroup(id,owner){
 if(!pool){const g=mem.groups.find(x=>x.id===id);if(!g||g.owner!==owner)return false;mem.groups=mem.groups.filter(x=>x.id!==id);return true}
 const r=await pool.query('DELETE FROM groups_data WHERE id=$1 AND owner=$2 RETURNING id',[id,owner]);return r.rowCount>0;
}
...
if(req.url==='/api/snapshot')return json(res,200,{users:await users(),messages:await allMessages(),groups:await allGroups()});if(req.url.startsWith('/api/groups/search?')){const q=new URL(req.url,'http://localhost').searchParams.get('username')?.toLowerCase()||'';const gs=await allGroups();return json(res,200,{group:gs.find(g=>g.username===q)||null})}
...
if(m.type==='clear-chat'){
 const target=String(m.username||'').replace(/^@/,'').trim().toLowerCase();
 if(!target||target===ws.user.username)return;
 await clearChat(ws.user.username,target);
 ws.send(JSON.stringify({type:'chat-cleared',username:target}));
 sendTo(target,{type:'chat-cleared',username:ws.user.username});
 return;
}
if(m.type==='group-delete'){
 const id=String(m.id||'');
 const ok=await deleteGroup(id,ws.user.username);
 if(!ok)return ws.send(JSON.stringify({type:'error',message:'Удалить группу может только её создатель'}));
 for(const [x] of clients)if(x.readyState===WebSocket.OPEN)x.send(JSON.stringify({type:'group-deleted',id}));
 return;
}
if(m.type==='group-create'){
 const name=String(m.name||'').trim().slice(0,50), gu=String(m.username||'').replace(/^@/,'').trim().toLowerCase();
 if(!name)return ws.send(JSON.stringify({type:'error',message:'Введите название группы'}));
 if(!/^[a-z0-9_.-]{3,24}group$/.test(gu))return ws.send(JSON.stringify({type:'error',message:'Юзер группы должен заканчиваться на group'}));
 const existing=(await allGroups()).find(g=>g.username===gu);if(existing)return ws.send(JSON.stringify({type:'error',message:'Юзер группы уже занят'}));
 const g={id:crypto.randomUUID(),name,username:gu,owner:ws.user.username,members:[ws.user.username],createdAt:Date.now()};
 await addGroup(g);for(const [x] of clients)if(x.readyState===WebSocket.OPEN)x.send(JSON.stringify({type:'group',group:g}));return
}
if(m.type==='group-join'){
 const id=String(m.id||'');const gs=await allGroups();const g=gs.find(x=>x.id===id);if(!g)return ws.send(JSON.stringify({type:'error',message:'Группа не найдена'}));
 if(!g.members.includes(ws.user.username))g.members.push(ws.user.username);
 if(pool)await pool.query('UPDATE groups_data SET members=$1 WHERE id=$2',[JSON.stringify(g.members),g.id]);else{const x=mem.groups.find(y=>y.id===g.id);if(x)x.members=g.members}
 ws.send(JSON.stringify({type:'group',group:g}));return
}
